...
export class SeasonsController {
  ...
  async findAll(@Res() res) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao buscar todas temporadas' });
    }
  }
  ...
  async create(@Res() res, @Body() createSeasonsDto: CreateSeasonsDto) {
    try {
      ...
      return res.status(HttpStatus.CREATED).send(season);
    } catch (error) {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao criar a temporada, ' + error.message });
    }
  }

  @Roles(UserType.Admin)
  @Put(':seasonId')
  async update(@Res() res, @Param('seasonId') seasonId: number, @Body() updateSeasonsDto: UpdateSeasonsDto) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao atualizar a temporada, ' + error.message });
    }
  }
  ...
  async remove(@Res() res, @Param('id') id: number) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao deletar a temporada, ' + error.message });
    }
  }
}
