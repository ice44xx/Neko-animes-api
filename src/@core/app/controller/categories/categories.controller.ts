...
export class CategoriesController {
  ...
  async findAll(@Res() res) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao buscar as categorias, ' + error.message });
    }
  }
  ...
  async create(@Res() res, @Body() createCategoriesDto: CreateCategoriesDto) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao criar a categoria, ' + error.message });
    }
  }

  @Roles(UserType.Admin)
  @Put(':id')
  async update(@Res() res, @Param('id') id: number, @Body() updateCategoriesDto: UpdateCategoriesDto) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao atualizar a categoria, ' + error.message });
    }
  }
  ...
  async remove(@Res() res, @Param('id') id: number) {
    try {
      ...
      return res.status(HttpStatus.INTERNAL_SERVER_ERROR).send({ message: 'Ocorreu um erro ao deletar a categoria, ' + error.message });
    }
  }
}
